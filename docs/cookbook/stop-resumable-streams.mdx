...
Store a `canceledAt` timestamp on the message (not the chat). When the user clicks stop, set it with a server action using the current message ID. The stream loop polls `canceledAt` and aborts the provider with a server-owned `AbortController`.
...
- You can call the `stopStream` server action from the client (e.g., via React Query)

## How it works

1. Client calls `stopStream` with the current `messageId`
...
```ts title="src/server/actions/chat.ts"
