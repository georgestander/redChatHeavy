...
function getFilePartsWithUrls() {
  return db
    .select({ file_url: part.file_url })
    .from(part)
    .where(and(eq(part.type, "file"), sql`${part.file_url} is not null`));
}

export async function getAllAttachmentUrls(): Promise<string[]> {
  try {
    const messages = await getMessagesWithAttachments();
    const fileParts = await getFilePartsWithUrls();
    const generatedImageParts = await getGeneratedImageParts();
    return [
      ...collectUrlsFromMessageAttachments(messages),
      ...collectUrlsFromGeneratedImages(generatedImageParts),
      ...collectUrlsFromFileParts(fileParts),
    ];
  } catch (error) {
...
function collectUrlsFromMessageAttachments(
...
function collectUrlsFromGeneratedImages(
  generatedImageParts: Array<{ tool_output: unknown }>
): string[] {
  const urls: string[] = [];
...
function collectUrlsFromFileParts(
  fileParts: Array<{ file_url: string | null }>
) {
  return fileParts
    .map((partWithFileUrl) => partWithFileUrl.file_url)
...
export async function canAccessAttachmentUrl({
  ...
  try {
    const accessScope = userId
      ? or(eq(chat.userId, userId), eq(chat.visibility, "public"))
      : eq(chat.visibility, "public");

    const attachmentMatch = sql<boolean>`exists (
      select 1
      from jsonb_array_elements(
        case
          when jsonb_typeof(${message.attachments}::jsonb) = 'array'
            then ${message.attachments}::jsonb
          else '[]'::jsonb
        end
      ) as attachment
      where attachment->>'url' = ${attachmentUrl}
    )`;
    ...
    const directAttachment = await db
      ...
      .from(message)
      .innerJoin(chat, eq(message.chatId, chat.id))
      .where(and(accessScope, attachmentMatch))
      .limit(1);

    if (directAttachment.length > 0) {
      return true;
    }

    const generatedImageToolName: ToolName = "generateImage";
    const partAttachment = await db
      .select({ id: chat.id })
      .from(part)
      ...
      .where(
        and(
          accessScope,
          or(
            and(eq(part.type, "file"), eq(part.file_url, attachmentUrl)),
            and(
              eq(part.tool_name, generatedImageToolName),
              sql`${part.tool_output}::jsonb ->> 'imageUrl' = ${attachmentUrl}`
            )
          )
        )
      )
    ...
    return partAttachment.length > 0;
  } catch (error) {
    console.error("Failed to check attachment visibility", error);
    throw error;
  }
}
...
async function deleteAttachmentsFromMessages(messages: DBMessage[]) {
  try {
    ...
    console.error("Failed to delete attachments from file storage:", error);
