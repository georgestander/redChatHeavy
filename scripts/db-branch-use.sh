...
if [ -z "$BRANCH_NAME" ]; then
  ...
  echo ""
  echo "Usage: pnpm db:branch:use <branch-name>"
  echo "       pnpm db:branch:use main  (switch to production)"
  exit 0
fi
...
BRANCH_URL=$(pnpm dlx neonctl connection-string "$BRANCH_NAME" 2>/dev/null | grep -E '^postgresql://') || {
  ...
  echo "$BRANCH_URL"
  echo ""
  echo "Available branches: pnpm db:branch:list"
  echo "Create branch: pnpm db:branch:create"
  exit 1
}
