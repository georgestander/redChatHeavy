...
import { parseMcpOAuthCallbackSearchParams } from "@/lib/mcp-search-params";

const log = createModuleLogger("mcp-oauth-callback");

export async function handleMcpOAuthCallbackRequest(request: Request) {
  ...
  } = parseMcpOAuthCallbackSearchParams(requestUrl);
...
export async function GET(request: Request) {
  return await handleMcpOAuthCallbackRequest(request);
}
